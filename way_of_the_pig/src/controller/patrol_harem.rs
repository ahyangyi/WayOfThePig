    ...
    fn buy<G: game::GameState, const P: usize>(&mut self, game: &mut G) {
        ...
        if game.colony_enabled() {
            if game.buy_colony::<P>() {
                ...
                return;
            } else if game.get_player::<P>().count_card(card::CardType::Patrol) >= 2 && game.buy_harem::<P>() {
                return;
            } else if game.buy_gold::<P>() {
                return;
            } else if game.colony_in_supply() <= 6 && game.buy_duchy::<P>() {
                return;
            } else if game.get_player::<P>().count_card(card::CardType::Patrol) * 4 < num_money::<G, P>(game) && game.buy_patrol::<P>()
            {
                return;
            } else {
                game.buy_silver::<P>();
