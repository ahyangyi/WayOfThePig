    ...
    fn buy<G: game::GameState, const P: usize>(&mut self, game: &mut G) {
        ...
        if game.colony_enabled() {
            if total_money::<G, P>(game) > 15 && game.buy_colony::<P>() {
                ...
                return;
            } else if game.get_player::<P>().count_card(card::CardType::Patrol) * 11 < num_money::<G, P>(game) && game.buy_smithy::<P>() {
                return;
            } else {
                game.buy_silver::<P>();
            }
            ...
            if total_money::<G, P>(game) > 15 && game.buy_province::<P>() {
                ...
                return;
            } else if game.get_player::<P>().count_card(card::CardType::Smithy) * 11 < num_money::<G, P>(game) && game.buy_smithy::<P>() {
                return;
            } else {
                game.buy_silver::<P>();
            }
