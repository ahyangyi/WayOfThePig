...
pub struct StaticGameState<'a, K: kingdom::Kingdom, RNG: rand::Rng + ?Sized, const N: usize> {
...
impl<'a, K: kingdom::Kingdom + Default, RNG: rand::Rng + ?Sized, const N: usize> StaticGameState<'a, K, RNG, N> {
    pub fn make(rng: &'a mut RNG) -> StaticGameState<'a, K, RNG, N> {
        let k = K::default();
        let ret: StaticGameState<'a, K, RNG, N> = StaticGameState {
...
impl<K: kingdom::Kingdom + Default, RNG: rand::Rng + ?Sized, const N: usize> GameState for StaticGameState<'_, K, RNG, N> {
